...
class List{
    ...
    public:
        ...
        void ListCopy(List* l){

            if(this->first!=NULL){
                Value v=this->first->val;
                ListElement* Lptr= new ListElement(v);
                ListElement* temp; 
                ListElement* oldFirst=Lptr; 
                //Lptr;
                temp=this->first;
                while(temp->next!=NULL){
                    temp=temp->next;
                    v=temp->val;
                    Lptr->next=new ListElement(v);
                    Lptr=Lptr->next;
        ...
        Value popFirst(){
            if(this->first==NULL)std::cout<<"WARNING LIST IS EMPTY ON CALLING OF POP\n";
